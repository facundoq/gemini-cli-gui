...
- **Session Management** - Native session resumption using Gemini CLI's built-in history tracking.
- **Model Selection** - Choose from multiple Gemini models including Gemini 3.1 Pro Preview.
- **Approval Modes** - Control tool execution with multiple levels: Default, Auto-edit, YOLO, and Plan.
- **Sandbox Mode** - Run tool executions in a secure, containerized environment for added safety.
- **Image Handling** - Upload and analyze images using native Gemini CLI file referencing (@ syntax).
...
Gemini CLI now supports multiple approval modes for tool execution:

- **Default (Prompt)**: Ask for confirmation for every tool call. This is the safest mode.
- **Auto-edit**: Automatically approve file modifications, but prompt for shell commands.
- **YOLO (Full Auto)**: Automatically approve all tool calls, including potentially destructive shell commands.
- **Plan (Read-only)**: Allow the model to research and read files, but block all modifications and executions.

**Recommendation**: Start with **Default** or **Auto-edit** and only use **YOLO** mode when you're confident in the model's actions or in non-critical environments. You can also enable **Sandbox Mode** for an extra layer of security.
