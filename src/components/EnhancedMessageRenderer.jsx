...
export const EnhancedMessageRenderer = ({ content, isDarkMode = true }) => {
  ...
  let summaryText = "";
  if (processedContent?.includes("Error: Experiments loaded") || processedContent?.includes("[AgentRegistry] User policy exists")) {
    isVerboseLog = true;
    summaryText = "System Configuration Logs";
  } else if (processedContent?.startsWith("Error: ") && processedContent.length > 500) {
    isVerboseLog = true;
    summaryText = "Verbose Error Log";
  }
  ...
  return (
    <div className="prose prose-sm max-w-none dark:prose-invert leading-relaxed">
      {isVerboseLog ? (
        <details className="group border border-gray-200 dark:border-gray-700 rounded-lg p-2 bg-gray-50 dark:bg-gray-800/50 mt-2 mb-2">
          <summary className="text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer outline-none select-none flex items-center justify-between">
            {summaryText}
            <span className="text-xs text-gray-500">Click to expand</span>
          </summary>
          <div className="mt-4 overflow-x-auto text-xs opacity-80">
            {markdownRenderer}
          </div>
        </details>
      ) : (
