        ...
        <div className="w-full">
          ...
          <div className="w-full">
            ...
            {message.isToolUse && !['Read', 'TodoWrite', 'TodoRead'].includes(message.toolName) ? (
              ...
              <div className={`text-sm ${message.type === 'error' ? 'text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border border-red-200 dark:border-red-800' : 'text-gray-700 dark:text-gray-300'}`}>
                {message.type === 'assistant' ? (
                  ...
                  <details className="group w-full max-w-full relative">
                    <summary className="cursor-pointer font-medium select-none outline-none flex items-center justify-between">
                      <span className="truncate pr-16 flex-1 text-red-800 dark:text-red-200">
                        {message.content.split('\n')[0].substring(0, 100) + (message.content.length > 100 || message.content.includes('\n') ? '...' : '')}
                      </span>
                      <span className="text-xs opacity-70">Click to expand ({message.content.split('\n').length} lines, {message.content.length} chars)</span>
                    </summary>
                    <div className="relative">
                      <button
                        ...
                        className="absolute top-2 right-2 px-2 py-1 text-xs bg-red-600 hover:bg-red-700 text-white rounded transition-colors z-10"
                      >
                        Copy
                      </button>
                      <div className="whitespace-pre-wrap select-all cursor-text pr-16 mt-3 text-xs overflow-x-auto border-t border-red-200 dark:border-red-800/50 pt-3">
                        {message.content}
                      </div>
                    </div>
                  </details>
...
function ChatInterface({ selectedProject, selectedSession, ws, sendMessage, messages, onFileOpen, onInputFocusChange, onSessionActive, onSessionInactive, onReplaceTemporarySession, onNavigateToSession, onShowSettings, autoExpandTools, showRawParameters, autoScrollToBottom }) {
  ...
  const [selectedModel, setSelectedModel] = useState(() => {
    try {
      const settings = JSON.parse(localStorage.getItem('gemini-tools-settings') || '{}');
      return settings.selectedModel || 'auto';
    } catch (e) {
      return 'auto';
    }
  });
      ...
      switch (latestMessage.type) {
        ...
        case 'gemini-response':
          ...
          if (Array.isArray(messageData.content)) {
            ...
            // Add regular text message by appending to previous if it's an assistant message
            setChatMessages(prev => {
              ...
              if (
                ...
                // Ensure proper spacing between concatenated messages if needed
                const prevContent = lastMessage.content;
                const newContent = messageData.content;
                let finalContent = prevContent;
        ...
        case 'gemini-error':
          ...
          setChatMessages(prev => {
            ...
            if (lastMessage && lastMessage.type === 'error') {
              // Only prepend 'Error: ' if the last message wasn't also reporting the start of an error
              // Actually, simply appending the raw error is usually cleaner for streams.
              // We'll assume the front-end string 'Error: ' was added to the first chunk.
              return [
                ...prev.slice(0, -1),
                {
                  ...lastMessage,
  ...
  const handleSubmit = async (e) => {
      ...
      try {
        ...
        if (savedSettings) {
          ...
          return {
            ...
            approvalMode: settings.approvalMode || (settings.skipPermissions ? 'yolo' : 'default'),
            useSandbox: settings.useSandbox || false,
            selectedModel: settings.selectedModel || 'auto'
          };
        }
      } catch (error) {
    ...
    sendMessage({
      ...
      options: {
        ...
        model: toolsSettings.selectedModel || 'auto',
        ...
        <div className={`p-2 sm:p-4 md:p-6 flex-shrink-0 ${isInputFocused ? 'pb-2 sm:pb-4 md:pb-6' : 'pb-16 sm:pb-4 md:pb-6'
          ...
          <div className="max-w-4xl mx-auto mb-3">
            <div className="flex items-center justify-center gap-3">
              <div className={`px-4 py-1.5 rounded-lg text-sm font-medium border transition-all duration-200 ${isYoloMode
