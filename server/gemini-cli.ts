...
async function spawnGemini(command, options = {}, ws) {
  return new Promise(async (resolve, reject) => {
    ...
    // Use native resume if sessionId is provided and we have a CLI mapping
    ...
    if (sessionId && !sessionId.startsWith('new-session-')) {
      ...
      if (session && session.cliSessionId) {
        ...
        // Fallback: try to use the UI sessionId directly if it's a UUID
        // (This helps if we imported sessions or if they match)
    ...
    // Clean the path by removing any control characters
    const cleanPath = typeof cwd === 'string' ? cwd.replace(/[\x00-\x1F\x7F]/g, '').trim() : process.cwd();
    const workingDir = cleanPath || process.cwd();
    // Debug - workingDir
    ...
    if (images && images.length > 0) {
      try {
        ...
        // Include the full image paths in the prompt using native @ syntax
        // Gemini CLI uses read_file internally when it sees @path/to/file
        if (tempImagePaths.length > 0 && command && command.trim()) {
          const imageAttachments = tempImagePaths.map(p => ` @${p}`).join('');
          
          // Update the command in args
          const promptIndex = args.indexOf('--prompt');
          if (promptIndex !== -1) {
            args[promptIndex + 1] = args[promptIndex + 1] + imageAttachments;
          }
        }
    ...
    // Always add debug flag to capture session ID
    args.push('--debug');
    ...
    const approvalMode = settings.approvalMode || (settings.skipPermissions ? 'yolo' : 'default');
    ...
    geminiProcess.stdout.on('data', (data) => {
      ...
      // Check for native CLI session ID in output
      const sessionIdMatch = rawOutput.match(/Session ID: ([a-f0-9-]{36})/i);
      if (sessionIdMatch && sessionIdMatch[1]) {
        const nativeId = sessionIdMatch[1];
        const uiSessionId = capturedSessionId || sessionId;
        if (uiSessionId) {
          sessionManager.setCliSessionId(uiSessionId, nativeId);
        }
      }
      ...
      const filteredLines = lines.filter(line => {
        ...
        if (line.includes('[DEBUG]') ||
          ...
          line.includes('Experiments loaded') ||
          line.includes('[AgentRegistry] User policy exists') ||
    ...
    geminiProcess.stderr.on('data', (data) => {
      ...
      if (errorMsg.includes('[DEP0040]') ||
        ...
        errorMsg.includes('Experiments loaded') ||
        errorMsg.includes('[AgentRegistry] User policy exists') ||
    ...
    geminiProcess.on('error', (error) => {
      ...
      if (error.code === 'ENOENT') {
        errorMessage = `Failed to start: "Error: spawn ${geminiPath} ENOENT". Please ensure the Gemini CLI is installed and in your PATH, or check your working directory (${workingDir}).`;
      }
